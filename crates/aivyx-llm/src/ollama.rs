...
impl OllamaProvider {
    pub fn new(base_url: String, model: String) -> Self {
        Self {
            // 600s read_timeout: large models (32B+) may take time to load into
            // VRAM on first request. read_timeout resets on each chunk, so active
            // inference stays alive indefinitely.
            inner: OpenAICompatibleProvider::new(None, model, base_url, "ollama".into(), 600),
        }
    }
}
